...
export class HubServer<TUser, TToken> extends ModuleBase<TUser, TToken> {
    ...
    private getEventDispatcherProperties ( instance: any ): string[] {
        ...
        for( const name of props.sort() ){
            try {
                const constructorExists = instance[name].constructor ? true :false
                const typeExists = instance[name]._type ? true : false

                if ( constructorExists && typeExists && (
                ...
                console.warn( `error in property name: ${name}` )
            }
        }
        return names
    }
